...
const Register = (props) => {
    ...
    const [passwordError, setPasswordError] = useState("Строка с поролем не может быть пустой");
    ...
    return (
        <div className='body__register'>
            ...
            <BaseInput onChange={e => passwordHandler(e)} onBlur={e => blurHandler(e)} value={password} invalid={passwordDirty && passwordError} name={"password"} type={"password"} placeholder={"Password"} />
            <span className={passwordError ? "error-span" : ""}>{(passwordDirty && passwordError) && (passwordError)}</span>

            <BaseButton disabled={!formValid && disable} styles={disable ? "modal-btn disable" : "modal-btn"}>Register</BaseButton>
        </div>
    );
};
